...
describe("Parse page", () => {
    ...
    it('base values', async () => {
        ...
        assert.equal(JSON.stringify(rewriteValue(parse.base.values)), '[{"key":"codeFile","value":"inherit","char":"\'"}]');
    });
    it('placeholders values', () => {
        GlobalSettings.development = true;
        parse.parseValues();
        assert.equal(JSON.stringify(rewriteValue(parse.values)), '[{"key":"insert","value":"\\n    moo\\n    "}]');
    });
    it('placeholders', () => {
        GlobalSettings.development = true;
        parse.parsePlaceHolder();
        assert.equal(JSON.stringify(parse.locations), '[{"name":"title","start":212,"end":10},{"name":"head","start":576,"end":9},{"name":"body","start":1371,"end":9}]');
    });
});
