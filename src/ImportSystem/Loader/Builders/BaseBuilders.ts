import { defaultVariables, GetScriptSettings, TransformJSC, TRANSFORM_MODULE } from "../../../Compilers/SWC/Settings.js";
...
function createTranspileEnvironment(script: string, path: PPath, params: string[], vars = {}){
    script = `require.url = "${
...
export async function transpileCode(content: string, templatePath: PPath, {params = [], isTypescript = false, template: codeTemplate}: {params?:string[], isTypescript?: boolean, template?: TranspilerTemplate}) {
...
export async function transpileStringTracker(content: StringTracker, templatePath: PPath, {easSyntax = true, params = [], isTypescript = false, template: codeTemplate}: {easSyntax?: boolean, params?:string[], isTypescript?: boolean, template?: TranspilerTemplate}) {
    const options: TransformOptions = {
