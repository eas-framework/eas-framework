...
import { ImportFile } from "../ImportFiles/Script.js";
import EasyFs from "../OutputInput/EasyFs.js";
import CompileState from "./CompileState.js";
import { isFileType } from "./FileTypes.js";
...
export async function debugSiteMap(Export) {
    return createSiteMap(Export, await scanFiles());
}
export async function createSiteMap(Export, state) {
    ...
    urls: //eslint-disable-next-line 
     for (let [url, type] of state.pages) {
        ...
        if (routing.ignoreTypes.find(ends => url.endsWith('.' + ends)) ||
